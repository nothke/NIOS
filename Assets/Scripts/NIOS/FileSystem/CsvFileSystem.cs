    ...
    public class CsvFileSystem : IFileSystem
    {
        ...
        public void DeleteDirectory(DirEntry dir)
        {
            ...
            if (!GetEntry(path, ref id))
                throw new DirectoryNotFoundException("'" + dir + "' doesnt exist");
            if (fileContents.ContainsKey(id)) // this is file
                throw new DirectoryNotFoundException("'" + dir + "' is a file, use delete file instead");
            entries.Remove(id);
            RemoveChildEntries(id);
            Save();
        }
        ...
        public void DeleteFile(FileEntry file)
        {
            ...
            if (!fileContents.ContainsKey(id)) // this is directory
                throw new FileNotFoundException("'" + file + "' is a directory, use delete directory instead");
            entries.Remove(id);
            fileContents.Remove(id);
            Save();
        }
        ...
        public Stream Open(FileEntry file, FileMode mode, FileAccess access, FileShare share)
        {
            ...
            if (entryExists)
            {
                if (MustNotExist(mode))
                    throw new IOException("file '" + file + "' already exists, mode: " + mode + ", access: " + access);
                if (!fileContents.ContainsKey(id)) // this is a directory, not a file
                    throw new FileNotFoundException("'" + file + "' is existing directory, canot open as file");
            ...
            {
                if (MustExist(mode))
                    throw new FileNotFoundException("file '" + file + "' doesnt exist, mode: " + mode + ", access: " + access);
